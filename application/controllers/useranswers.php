...
class Useranswers extends CI_Controller {
  ...
  { 
       ...
       if(!$userid)
        ...
        $config = array();
        $config["base_url"] = base_url() . "index.php/useranswers/index/$userid";
...
//        $config['full_tag_open'] = '<p>';
        ...
        $data['useranswer_list'] =  $this->process->user_answerlist($userid,$config["per_page"], $page);
...
//                $data['totalquestion'] = $this->process->get_totalquestion($userid);  
                 $data['totalanswered'] = $this->process->useranswerlist_count($userid);  
//                $data['totalanswered'] = $this->process->get_totalanswered($userid);  
		...
		$this->load->view('viewuseranswers',$data);
